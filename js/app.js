...
} while (calc.isCorrectAction(action));function Calculator() {
    this.actions = ['+', '-', '*', '/', '^'];
    this.history = [];
}

function isCorrectValues(num1, num2) {
    return (Number.isNaN(num1)) || Number.isNaN(num2) ? false : true;
}
...
// function addToHistory(num1, num2, action, result) {
//     calc.history.push(`${num1} ${action} ${num2} = ${result}`) 
// }
...
do {
    ...
    if (isCorrectAction) {
        ...
        // addToHistory(number1, number2, action, result);
    }
