...
Calculator.prototype.verifyInputs = function (num1, num2) {
  const [num1Converted, num2Converted] = this.covertCommaToDot(num1, num2);
  return !isNaN(num1Converted) && !isNaN(num2Converted);
};

Calculator.prototype.errorGenerator = function (num1, num2) {
  const [num1Converted, num2Converted] = this.covertCommaToDot(num1, num2);
  let error = "";
  if (isNaN(num1Converted) || !num1Converted)
    error += "Pierwszy argument nie jest liczbą\n";
  if (isNaN(num2Converted) || !num2Converted)
...
Calculator.prototype.addToHistory = function (num1, num2, action, result) {
  const [num1Converted, num2Converted] = this.covertCommaToDot(num1, num2);
  const record = `${num1Converted} ${action} ${num2Converted} = ${result}`;
